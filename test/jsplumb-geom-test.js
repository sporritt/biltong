...
window.testSuite = function() {
    ...
    test("quadrant, quadrant 1", function() {
		var p1 = [2,0], p2 = [3,-1], s = jsPlumbGeom.quadrant(p1,p2);
    ...
    test("quadrant, quadrant 3", function() {
		var p1 = [416.7166748046875, 116.13333129882812], 
            ...
            s = jsPlumbGeom.quadrant(p1,p2);
        
		equal(s, 3, "quadrant 3 correct");
	});    
    
    test("quadrant, quadrant 3", function() {
		var p1 = [2,0], p2 = [1,1], s = jsPlumbGeom.quadrant(p1,p2);
		equal(s, 3, "quadrant 3 correct");
	});        
    ...
    test("quadrant, quadrant 1 edge case", function() {
		var p1 = [2,0], p2 = [3,0], s = jsPlumbGeom.quadrant(p1,p2);
		equal(s, 1, "quadrant 1 correct");
	});        
    
    test("quadrant, quadrant 2 edge case", function() {
		var p1 = [2,0], p2 = [2,1], s = jsPlumbGeom.quadrant(p1,p2);
		equal(s, 2, "quadrant 2 correct");
	});   
    
    test("quadrant, quadrant 3 edge case", function() {
		var p1 = [2,0], p2 = [1,0], s = jsPlumbGeom.quadrant(p1,p2);
		equal(s, 4, "quadrant 4 correct");
	});        
    ...
    test("perpendicularLineTo, quadrant 1", function() {
        var p1 = {x:2, y:2}, p2={x:3, y:1}, m = jsPlumbGeom.gradient(p1, p2),
            l = jsPlumbGeom.perpendicularLineTo(p1, p2, 2 * Math.sqrt(2));
	...
	test("perpendicularLineTo, quadrant 2", function() {
        var p1 = {x:2, y:2}, p2={x:3, y:3}, m = jsPlumbGeom.gradient(p1, p2),
            l = jsPlumbGeom.perpendicularLineTo(p1, p2, 2 * Math.sqrt(2));
    ...
    test("perpendicularLineTo, quadrant 3", function() {
        var p1 = {x:2, y:2}, p2={x:1, y:3}, m = jsPlumbGeom.gradient(p1, p2),
            l = jsPlumbGeom.perpendicularLineTo(p1, p2, 2 * Math.sqrt(2));
    ...
    test("perpendicularLineTo, quadrant 4", function() {
        var p1 = {x:2, y:2}, p2={x:1, y:1}, m = jsPlumbGeom.gradient(p1, p2),
            l = jsPlumbGeom.perpendicularLineTo(p1, p2, 2 * Math.sqrt(2));
    ...
    test("intersects, with intersection", function() {
    	...
    	ok(jsPlumbGeom.intersects(r1, r2), "r1 and r2 intersect");
    });
    test("intersects, with no intersection", function() {
    	var r1 = { x: 2, y:2, w:4, h:6},
    		r2 = { x: 13, y:4, w:3, h:3};

    	ok(!jsPlumbGeom.intersects(r1, r2), "r1 and r2 do not intersect");
    });
    ...
    test("theta", function() {
        ...
        var t2 = jsPlumbGeom.theta([x1,y1],[x2,y2]),
            t3 = jsPlumbGeom.theta([x1,y1],[x3,y3]),
            t4 = jsPlumbGeom.theta([x1,y1],[x4,y4]),
            t5 = jsPlumbGeom.theta([x1,y1],[x5,y5]),
            t6 = jsPlumbGeom.theta([x1,y1],[x6,y6]),
            t7 = jsPlumbGeom.theta([x1,y1],[x7,y7]),
            t8 = jsPlumbGeom.theta([x1,y1],[x8,y8]),            
            t9 = jsPlumbGeom.theta([x1,y1],[x9,y9]);            
